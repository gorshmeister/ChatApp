...
class StreamAllFragment : Fragment(R.layout.fragment_channels_stream) {
    ...
    private fun onTopicClick(topicId: Int) {
        ...
        chatFragment.arguments = bundleOf(
            ChannelsFragment.STR_NAME to (stream as StreamUi).name,
            ChannelsFragment.TPC_NAME to (topic as TopicUi).name
        )
...
//            .doOnNext { binding.shimmerChannels.startShimmer() }
    ...
    private fun initViews() {
        with(binding) {
            ...
            parentFragmentManager.setFragmentResultListener(STREAM_SEARCH, this@StreamAllFragment) { _, result ->
