    ...
    override fun bind(
        ...
        return actions.ofType(StreamAction.OnTopicClick::class.java)
            .flatMap<StreamAction> { action ->
                val topic = action.items.filterIsInstance<TopicUi>().find { it.id == action.topicId }
                val streamName = action.items.filterIsInstance<StreamUi>().find { it.id == topic?.streamId }?.name
